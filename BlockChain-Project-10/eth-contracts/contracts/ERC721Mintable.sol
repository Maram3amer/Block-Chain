...
contract Pausable is Ownable{
    ...
    event WhenNotPaused(address _address);
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public returns(address) {
        // TODO require the given address to not be the owner of the tokenId
        address owner = ownerOf(tokenId);
        require(owner != to, "Must be not the owner of tokenId");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == _tokenOwner[tokenId] || isApprovedForAll(msg.sender, owner) == true);
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        ...
        require(_exists(tokenId), "token approval not exists");
        return _tokenApprovals[tokenId];
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId) == true, "Not Approved");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId)== true ,"token not exists");
...
contract CustomERC721Token is ERC721Metadata{
    ...
    function mint(address to, uint256 tokenId) public{
        require(msg.sender == getOwner(), "not contract owner");
        super._mint(to, tokenId);
        super.setTokenURI("https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/", tokenId);
    }
